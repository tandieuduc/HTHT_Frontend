...
const heThongs = [
  { id: 1, ten: "Bộ phận một cửa" },
  { id: 2, ten: "Quản lý văn bản" },
  { id: 3, ten: "Hệ thống A" },
  { id: 4, ten: "Hệ thống B" },
  { id: 5, ten: "Hệ thống C" },
];
...
// Hiển thị form
const addNewHeThong = ({ ma, ten }) => {
  heThongs.push({ id: heThongs.length + 1, ma, ten });
...
const filteredHeThongs = computed(() => {
  if (!searchKeyword.value) return heThongs;
  return heThongs.filter(pb => pb.ten.toLowerCase().includes(searchKeyword.value.toLowerCase()));
})
...
const pagedHeThongs = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value;
  return filteredHeThongs.value.slice(start, start + pageSize.value);
})
</script>
  ...
  <div class="p-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <CaterForm 
        :show="showAddModal" 
        title="Thêm hệ thống" 
        ma-label="Mã hệ thống" 
        ten-label="Tên hệ thống"
        @close="showAddModal = false" @submit="addNewHeThong" />
    </div>
      ...
      <table class="table custom-table table-hover align-middle mb-0">
        <thead class="table-light">
          <tr>
            ...
            <th id="table-header">Tên hệ thống</th>
            <th id="table-header">Trạng thái</th>
            <th id="table-header">Thao tác</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in pagedHeThongs" :key="item.id" :class="{ 'table-info': index % 2 === 0 }">
    ...
    <Pagination :total-items="filteredHeThongs.length" :current-page="currentPage" :page-size="pageSize"
