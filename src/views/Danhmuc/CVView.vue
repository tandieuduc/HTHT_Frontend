...
const chucVus = [
  { id: 1, ten: "Giám đốc" },
  { id: 2, ten: "Phó giám đốc" },
  { id: 3, ten: "Trưởng phòng" },
  { id: 4, ten: "Quản lý" },
  { id: 5, ten: "Nhân viên" },
];

// Hiển thị form
const addNewChucVu = ({ ma, ten }) => {
  chucVus.push({ id: chucVus.length + 1, ma, ten });
...
const filteredChucVus = computed(() => {
  if (!searchKeyword.value) return chucVus;
  return chucVus.filter(pb => pb.ten.toLowerCase().includes(searchKeyword.value.toLowerCase()));
})
...
const pagedChucVus = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value;
  return filteredChucVus.value.slice(start, start + pageSize.value);
})
</script>
  ...
  <div class="p-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <CaterForm 
        ...
        title="Thêm chức vụ" 
        ma-label="Mã chức vụ" 
        ten-label="Tên chức vụ"
        @close="showAddModal = false" @submit="addNewChucVu" />
    </div>
      ...
      <table class="table custom-table align-middle mb-0">
        <thead>
          <tr>
            ...
            <th id="table-header">Tên chức vụ</th>
            <th id="table-header">Trạng thái</th>
            <th id="table-header">Thao tác</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in pagedChucVus" :key="item.id" :class="{ 'table-info': index % 2 === 0 }">
    ...
    <Pagination :total-items="filteredChucVus.length" :current-page="currentPage" :page-size="pageSize"
